...
export default function ServicesPage() {
  return (
    <section style={{ background: "#fff" }}>
      {/* Inline styles guarantee 2-column on desktop >=1000px */}
      <style>{`
        ...
        .svc-card { background:#f8fafc; border:1px solid #e6eef9; border-radius:10px; padding:18px; display:flex; gap:14px; align-items:flex-start; transition: transform .18s ease, box-shadow .18s ease; }
        ...
        /* FORCE 2 columns on desktop widths >= 1000px */
      ...
      <div className="svc-wrap">
        <div className="svc-head">
          ...
          <div className="svc-sub">
            Practical, end-to-end support for MBBS aspirants — admission guidance, documentation, counselling and post-admission support.
          </div>
        </div>
        ...
        <div className="services-grid">
          {SERVICES.map((s) => {
            const Icon = s.Icon;
            return (
              <article key={s.id} className="svc-card" aria-labelledby={`${s.id}-title`}>
                <div className="icon-wrap"><Icon /></div>

                <div style={{ flex: 1 }}>
                  <div className="svc-title-row">
                    <div id={`${s.id}-title`} className="svc-card-title">{s.title}</div>
                    {s.note && (
                  ...
                  <div className="actions">
                    <a className="learn" href={`#${s.id}`}>Learn more →</a>
                    <button className="btn" aria-label={`Request ${s.title}`}>Request</button>
                  </div>
                </div>
              </article>
            );
